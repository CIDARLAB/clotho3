"use strict";
angular.module("clotho.fullPackage", ["clotho.foundation", "clotho.commandbar", "clotho.webapp", "clotho.editor", "clotho.interface", "clotho.trails", "clotho.dna", "clotho.construction"]), angular.module("clothoRoot", ["clotho.fullPackage"]).config(["$routeProvider", "$locationProvider", function(a, b) {
  b.html5Mode(!1).hashPrefix("!"), a.when("/", {
    ...
    reloadOnSearch: !1,
    resolve: {
      deps: ["codemirrorLoader", function(a) {
        return a.loadMain()
      }]
    }
  }).when("/executor", {
    ...
    reloadOnSearch: !1,
    resolve: {
      trail: ["Clotho", "$q", "$http", "$route", "$location", "Trails", function(a, b, c, d, e, f) {
        if (angular.isUndefined(d.current.params.id)) return b.when(null);
        var g = b.defer();
        return a.get(d.current.params.id).then(function(a) {
          f.compile(a).then(function(b) {
            d.current.$$route.title = a.name, g.resolve(b)
          })
        }, function() {
          e.path("/trails")
        }), g.promise
    ...
    reloadOnSearch: !1,
    resolve: {
      trail: ["$q", "$http", "$route", "Trails", function(a, b, c, d) {
        var e = a.defer();
        return b.get("models/org.clothocad.trails.constructionFiles.json").then(function(a) {
          d.compile(a.data).then(function(a) {
            e.resolve(a)
          })
        }), e.promise
      }]
  ...
  a.$on("$routeChangeSuccess", function(a, b) {
    ...
    c.document.title = "Clotho" + (angular.isDefined(d) ? " | " + d : "")
  }), a.interfaceConfig = d
}]);
