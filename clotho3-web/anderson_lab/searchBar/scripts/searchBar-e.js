...
(function ($) {
    $(function () {
         ...
         *
         */
        var dataSource = "http://www.bussedesignstage.com/ajax/general-error.php";
         ...
         */
            ...
            if (hasInfoPane) {
                ...
                // construct the header and insert title
                var $infoPaneHeader = $("<div class='header'>").append("<h2>" + item.searchTerm + "</h2>");
                ...
                if (item.infoPane.hasOwnProperty('command')) {
                    ...
                    $infoPaneHeader.append("<p class='command'>" + commandString + "</p>");
                ...
                if (item.infoPane.hasOwnProperty('executeButton')) {
                    $infoPaneHeader.append("<a class='execute button' href='" + item.infoPane.executeButton.linkURL + "'>" + item.infoPane.executeButton.linkText + "</a>");
                } // end build execute button

                // add the header to the info pane
                var $infoPane = $("<div class='infoPane'>").append($infoPaneHeader);

                // create the info pane content
                if (item.infoPane.hasOwnProperty('infoContent')) {

                    var $infoPaneContent = $("<div class='content'>");
                    // var firstInfoPane = true;
                    ...
                    for (key in item.infoPane.infoContent) {
                        ...
                        infoBlockHeaders += "<a class='infoBlockHeaderButton author'><span>Author:</span>" + item.infoPane.infoContent[i].header.author.linkText + "</a>";
                        infoBlockHeaders += "</div>";
                        i++;
                    }
                    // Add all available block headers to the content pane
                    $infoPaneContent.append(infoBlockHeaders);
                    ...
                    // create the info block content container
                    var $infoBlockContent = $("<div id='blockContent'></div>");

                    // create the info block content header and add the rendered items
                    var temp = $("<div class='infoBlockContentHeader'></div>");
                    temp.append(renderedItems[0]);
                    // insert the info block content header
                    $infoBlockContent.append(temp);

                    // insert the content
                    // first wrap list in div container
                    var $contentContainer = $("<div class='infoBlockContentBody'>");
                    $contentContainer.append(renderedItems[1]);
                    $infoBlockContent.append($contentContainer);
                    // insert the info block content into the info pane content
                    $infoPaneContent.append($infoBlockContent);
                }

                $infoPane.append($infoPaneContent);
